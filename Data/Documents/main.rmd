  ...
  for(i in 1:dim(coint)[1]){
    ...
    if(coint$long_wti[i] == 1) {
      ...
      if(sign(coint$position_wti[i-1]) >= 0) {
        ...
        coint$position_wti[i] <- coint$position_wti[i-1] + 1*scale
        coint$position_brent[i] <- coint$position_brent[i-1] + x_val*scale
      }

      # Otherwise, we need to buy enough to make our holdings long.
