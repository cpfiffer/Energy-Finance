...
sharp <- function(returns) {
  excess = returns - risk_free$daily
  ri = mean(excess, na.rm = TRUE)
  s = sd(excess, na.rm = TRUE)
  return(ri/s)
}

trade <- function(current, target, price_cur, ba) {
  ...
  cashflow = -change*price_cur-((ba/2)*abs(change))
  #print(paste("The transaction cost for a trade of ", change, " is ", ((ba/2)*abs(change))))
...
trade_determine2 <- function(df, stdevs=1, cash = 1000, scale = 1) {
  ...
  coint$b.spread <- df$b.spread
  coint$w.spread <- df$w.spread
  
  coint <- do_trades(coint, stdevs, cash, scale)
  return(coint)
  
}

buy_and_hold <- function(df, stdevs=1, cash = 1000, scale = 1) {
  y = df$wti
  x = df$brent
  ...
  for(i in 2:dim(coint)[1]) {
    ...
    baw = coint$w.spread[[i]]
    bab = coint$b.spread[[i]]
    ...
    if(i==1) {
      ...
      cashflow = trade(cb, bt, coint$brent[[i]], bab) # Next, record trade
      # We give trade (for example)
      # cb = -12
      # bt = 0
      # price_cur = 51.54
      ...
      # Update WTI holdings.
      cashflow = trade(cw, wt, coint$wti[[i]], baw) # Next, record trade
      coint$trade_made[i] <- 1 # Then we update our cash.
      coint$cash[i] <- coint$cash[[i]] + cashflow
    ...
    if( (coint$long_brent[[i]] == 0) & (coint$long_wti[[i]] == 0))  {
    ...
    # Otherwise, do something else.
      ...
      if((coint$long_brent[[i]] == 1) & (coint$long_wti[[i]] == 0)) {
        ...
        else if (coint$position_brent[[i]] < 0){
          bt = 0
          wt = 0
      ...
      if((coint$long_wti[[i]] == 1) & (coint$long_brent[[i]] == 0)) {
        ...
        else if (coint$position_wti[[i]] < 0){
          bt = 0
          wt = 0
...
do_trades <- function(df, stdevs=1,cash=1000,scale=1) {
